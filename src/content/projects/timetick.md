---
title: Timetick — advanced IoT testing & monitoring tool
name: Timetick
slug: timetick
order: 4
tagline: AI-driven low-code platform for behavioral IoT testing — 100+ screens
roles:
  - UX architecture
  - UI design
  - Design system
years: 2023 — 2025
role: Senior Product Designer
cover: ../../assets/covers/timetick.png
coverAlt: TimeTick monitoring dashboard on a laptop
summary: >-
  An AI-driven low-code platform for behavioral testing and monitoring of IoT
  software, built for QA, engineers, and business users. Sole designer on an
  early-stage startup where structure changed every few days: daily design
  sprints with stakeholders and engineers, a custom system on top of MUI, and
  100+ screens turned into a full clickable prototype.
screens:
  - src: ../../assets/projects/timetick/timeline.png
    alt: TimeTick simulators timeline with scheduled device runs
  - src: ../../assets/projects/timetick/simulator.png
    alt: TimeTick device page with status, metrics, map, and run timeline
---
